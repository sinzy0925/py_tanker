...
マーカーは API のピンではなく、取得した地図上に船名の先頭 2 文字（例: TOWADA → TO）を赤丸で囲み白文字で描画する。
同一座標が複数ある場合は円状にずらし、略号が先の船の赤丸の内に「MA」「×3」のように2行で表示する。
...
def _build_ship_color_map(points: list[tuple[str, str, float, float]]) -> dict[str, tuple[int, int, int]]:
    """
    ...
    ship_ids = sorted({(sid or "").strip() for sid, _nm, _la, _lo in points})
...
def split_overlapping_points(
    points: list[tuple[str, str, float, float]],
) -> tuple[list[tuple[str, str, float, float]], dict[int, int]]:
    """
    ...
    positions: list[tuple[float, float]] = [(p[2], p[3]) for p in points]
    ...
    for _key, indices in groups.items():
        ...
        anchor_idx = min(
            indices,
            key=lambda i: (name_prefix2(points[i][1], points[i][0]), points[i][0]),
        )
        anchor_cluster[anchor_idx] = n

    display = [
        (points[i][0], points[i][1], positions[i][0], positions[i][1]) for i in range(len(points))
    ]
...
def _draw_name_labels_on_map(
    ...
    *,
    ...
    """各船位置の中央に船名先頭 2 文字を赤丸で囲んで白文字で描く。重複アンカーは ×n を同じ丸内の下段に入れる。"""
    from PIL import ImageDraw
    ...
    for i, (sid, name, lat, lon) in enumerate(points):
        ...
        if n_dup is not None and n_dup >= 2:
            line2 = f"×{n_dup}"
            bbox1 = draw.textbbox((0, 0), label, font=font)
            bbox2 = draw.textbbox((0, 0), line2, font=font_hint)
            h1 = bbox1[3] - bbox1[1]
            h2 = bbox2[3] - bbox2[1]
            w1 = bbox1[2] - bbox1[0]
            w2 = bbox2[2] - bbox2[0]
            gap_lines = max(1, scale // 2)
            total_h = h1 + gap_lines + h2
            total_w = max(w1, w2)
            ...
            y1_line = sy - total_h / 2.0 + h1 / 2.0
            y2_line = sy - total_h / 2.0 + h1 + gap_lines + h2 / 2.0
            draw.text(
                (sx, y1_line),
                label,
                font=font,
        ...
        tw = bbox[2] - bbox[0]
        th = bbox[3] - bbox[1]
        r = int(math.ceil(math.hypot(tw / 2.0, th / 2.0) + pad))
        x0, y0 = int(round(sx - r)), int(round(sy - r))
        x1, y1 = int(round(sx + r)), int(round(sy + r))
        draw.ellipse(
            [x0, y0, x1, y1],
...
def main() -> int:
    ...
    for sid, name, lat, lon in sorted(points, key=lambda p: (p[2], p[3])):
        abbr = name_prefix2(name, sid)
        print(f"  [{abbr}] {name} ({sid}) {lat:.5f},{lon:.5f}")
    return 0
