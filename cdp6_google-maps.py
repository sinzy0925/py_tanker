...
画像が大きすぎる（例: 1MB 超・高解像度）と Google 側の取得に失敗し、デフォルトの赤ピンだけになることがある。
その場合は make_static_map_icon.py で長辺 256px 程度に縮小した PNG をリポジトリに置き、その raw URL を指定する。

同一アイコンは 1 つの markers に icon:URL|座標1|座標2|... とまとめてリクエストする（本スクリプトの実装）。
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default="",
        metavar="HTTPS_URL",
        help="全マーカーに使う画像の公開 HTTPS URL（小さめ PNG 推奨・大きいと取得失敗で赤ピンになる）",
    )
    p.add_argument(
        "--marker-icon-anchor",
        default="bottom",
...
def build_static_map_url(
    *,
    ...
    if use_icon:
        # 同一 icon を複数 markers= で繰り返すと、画像 URL をリクエストごとに取得し直し失敗しやすい（デフォルト赤ピンにフォールバック）。
        # 公式どおり 1 つの markers に icon:URL|loc1|loc2|... とまとめる（画像取得は 1 回相当）。
        icon_u = marker_icon_url.strip()
        anc = marker_icon_anchor.strip() or "bottom"
        locs = "|".join(f"{lat:.6f},{lon:.6f}" for _sid, _name, lat, lon in points)
        marker_spec = f"anchor:{anc}|icon:{icon_u}|{locs}"
